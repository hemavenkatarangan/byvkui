...
module.exports = {
  ...
  getEventsForKendra: async (req, res) => {
    ...
    if (!events) {
      return res.status(404).send({
        status_code: "403",
        status_message:
          "Events not found with kendra id " + req.params.kendraId,
      });
    }
  ...
  replaceEvent: async (req, res) => {
    ...
    const resultEvent = await Event.findByIdAndUpdate(eventId, event, {
      new: true,
    });
  ...
  removeEvent: async (req, res) => {
    ...
    res
      .status(200)
      .json({ message: "deleted the event successfully", deletedEvent: event });
  },
};
