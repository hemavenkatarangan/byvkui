...
module.exports = {
  allPrograms: async (req, res, next) => {
    try {
      ...
      response.result = [{}];
      response.error = errorDetail.getErrorDetail(
        err.name,
  ...
  allActivePrograms: async (req, res, next) => {
    try {
      const programs = await Program.find({ status: { $ne: "INACTIVE" } }).sort(
  ...
  getProgramByCourse: async (req, res) => {
    ...
    const program = await Program.find({ course: courseId }).sort({
      program_start_date: -1,
    });
    if (!program) {
      response.status_code = "404";
  ...
  changeStatusForProgram: async (req, res) => {
    try {
      ...
      response.status_message =
        "Event could not be changed with status " + req.params.status;
      response.result = null;
      res.status(403).json(response);
    }
  },
